...
@Mapper
public interface HeBeiStudentMapper {

    @Select("select * from hebeistudent where phone=#{phone}")
    public Student selectStudentByIphone(String phone);

    @Select("select * from hebeistudent where snum=#{snum}")
    ...
    @Select("select number from hebeistudent where campusid=#{id} order by number desc limit 0,1")
    public Integer selectXuHao(Integer id);

    @Update("update hebeistudent set campus=#{campus},campusid=#{campusid},schoolname=#{schoolname},sname=#{sname},sex=#{sex},major=#{major},phone=#{phone}" +
